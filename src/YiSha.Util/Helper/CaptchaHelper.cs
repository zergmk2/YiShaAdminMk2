...
namespace YiSha.Util.Helper
{
    public class CaptchaHelper
    {
        ...
        public static Tuple<string, int> GetCaptchaCode()
        {
            ...
            var operatorChar = operators[random.Next(0, operators.Length)];
            switch (operatorChar)
            {
                case '+':
                    value = first + second;
        ...
        public static byte[] CreateCaptchaImage(string randomCode)
        {
            ...
            var mapwidth = randomCode.Length * 16;
            var map = new Bitmap(mapwidth, 28); //创建图片背景
            ...
            using (var format = new StringFormat(StringFormatFlags.NoClip))
            {
                ...
                //验证码旋转，防止机器识别
                var chars = randomCode.ToCharArray(); //拆散字符串成单字符数组
                foreach (var t in chars)
                {
                    var findex = random.Next(5);
                    using (var font = new Font(fonts[findex], 14, FontStyle.Bold)) //字体样式(参数2为字体大小)
                    {
