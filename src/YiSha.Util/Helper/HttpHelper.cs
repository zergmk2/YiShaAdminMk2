...
namespace YiSha.Util.Helper
{
    ...
    public class HttpHelper
    {
        ...
        public static string HttpPost(string posturl, string postData,
            string contentType = "application/x-www-form-urlencoded", int timeout = 10 * 1000)
        {
        ...
        public static string CreateAutoSubmitForm(string url, Dictionary<string, string> data, Encoding encoder)
        {
            ...
            html.AppendFormat("<meta http-equiv=\"Content-Type\" content=\"text/html; charset={0}\" />",
                encoder.BodyName);
            html.AppendLine("</head>");
            html.AppendLine("<body onload=\"OnLoadSubmit();\">");
            html.AppendFormat("<form id=\"pay_form\" action=\"{0}\" method=\"post\">", url);
            foreach (var kvp in data)
                html.AppendFormat("<input type=\"hidden\" name=\"{0}\" id=\"{0}\" value=\"{1}\" />", kvp.Key,
                    kvp.Value);
            ...
            try
            {
                ...
                using (response = (HttpWebResponse) request.GetResponse())
                {
                    ...
                    if (response.ContentEncoding != null &&
                        ...
                        //.net4.0以下写法
                        _stream = GetMemoryStream(new GZipStream(response.GetResponseStream(),
                    ...
                    if (encoding == null)
                    {
                        ...
                        charter = charter.Replace("\"", string.Empty).Replace("'", string.Empty)
        ...
        private void SetRequest(HttpItem httpItem)
        {
            ...
            //设置最大连接
            if (httpItem.Connectionlimit > 0) request.ServicePoint.ConnectionLimit = httpItem.Connectionlimit;
        }
        ...
        private void SetCer(HttpItem objhttpItem)
        {
            if (!string.IsNullOrEmpty(objhttpItem.CerPath))
            {
                ...
                //创建证书文件
                var objx509 = new X509Certificate(objhttpItem.CerPath);
                //添加到请求里
        ...
        private void SetPostData(HttpItem objhttpItem)
        {
            ...
            if (request.Method.Trim().ToLower().Contains("post"))
            {
                ...
                {
                    var r = new StreamReader(objhttpItem.Postdata, encoding);
    ...
    public class HttpResult
    {
        ...
        //header对象
        public WebHeaderCollection Header { get; set; } = new();
    }
